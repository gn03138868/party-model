...
class SievePredictor:
    def __init__(self, model_path, patch_size=128, model_patch_size=16, threshold=0.4):
        """
        初始化預測器
    ...
    def predict_single(self, image_path, output_path):
        ...
        batch_tensor = torch.cat(patches, dim=0).to(self.device)
        with torch.no_grad():
...
if __name__ == '__main__':
    try:
        predictor = SievePredictor(
            model_path='outputs/models/sievenet_epoch100.pth',  # 根據實際情況調整模型路徑
            patch_size=128,
            model_patch_size=16,
            threshold=0.4
        )
