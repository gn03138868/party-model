...
class SievePredictor:
    def __init__(self, model_path, patch_size=224, model_patch_size=16, threshold=0.4):
        """
        初始化預測器
        :param model_path: 模型權重路徑
        :param patch_size: 輸入 patch 尺寸（需與訓練時一致）
        :param model_patch_size: 模型內 ViT 切割圖像的 patch 尺寸
        ...
        # 初始化模型，將 patch_size 與 model_patch_size 一併傳入
        self.model = TransUNet(input_size=patch_size, model_patch_size=model_patch_size).to(self.device)
        try:
            checkpoint = torch.load(model_path, map_location=self.device)
            # 若 checkpoint 為字典則取出 state_dict
...
if __name__ == '__main__':
    try:
        predictor = SievePredictor(
            model_path='outputs/models/sievenet_epoch400.pth',  # 請根據實際情況調整模型路徑
            patch_size=224,
            model_patch_size=16,
            threshold=0.4
        )
